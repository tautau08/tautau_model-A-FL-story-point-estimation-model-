"""
server_phase2.py -- Flower server orchestrator for Phase 2 (FedAvg).
...
def main():
    ...
    if _round_results:
        ...
        print(f"\n  [5/5] Saving Phase 2 global models to disk ...")
    joblib.dump(mlp, FEDAVG_MLP_PATH)
    lstm.save(FEDAVG_LSTM_PATH)
    print(f"         Saved MLP to {FEDAVG_MLP_PATH.name}")
    print(f"         Saved LSTM to {FEDAVG_LSTM_PATH.name}")
    print("=" * 60)
